...
from .diffKD_tools.diffkd_modules import AutoEncoder, DiffusionModel, NoiseAdapter, DDIMPipeline
from .diffKD_tools.scheduling_ddim import DDIMScheduler
...
class DiffKD(nn.Module):
    def __init__(
        ...
        if use_ae:
            ...
            self.ae = AutoEncoder(teacher_channels, ae_channels).cuda()
            teacher_channels = ae_channels

        # transform student feature to the same dimension as teacher
        self.trans = nn.Conv2d(student_channels, teacher_channels, 1).cuda()
        # diffusion model - predict noise
        self.model = DiffusionModel(channels_in=teacher_channels, kernel_size=kernel_size).cuda()
        self.scheduler = DDIMScheduler(num_train_timesteps=num_train_timesteps, clip_sample=False,
                                       beta_schedule="linear")
        self.noise_adapter = NoiseAdapter(teacher_channels, kernel_size).cuda()
