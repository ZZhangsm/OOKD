...
def kd_loss(logits_student, logits_teacher, temperature, reduce=True):
    log_pred_student = F.log_softmax(normalize(logits_student) / temperature, dim=1)
    pred_teacher = F.softmax(normalize(logits_teacher) / temperature, dim=1)
    if reduce:
...
class LNKD(torch.nn.Module):
    def __init__(self):
        super(LNKD, self).__init__()
